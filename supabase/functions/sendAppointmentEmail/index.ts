...
function buildICS(opts) {
  const { title, description, start, end, location = "", organizerEmail, organizerName = "Organizer", attendees = [], uid = `${Date.now()}@advisoryplus.it`, sequence = 0 } = opts;
  const lines = [
    "BEGIN:VCALENDAR",
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { to_client_email, cc_advisor_email, cliente_nome, advisor_nome, ts_iso, durata_minuti = 60, modalita, note = "", location = "", subject: subjectIn, title: titleIn } = body;
    ...
    if (isNaN(start.getTime())) {
      return new Response(JSON.stringify({
        error: "ts_iso non è una data valida (ISO 8601)."
      }), {
        status: 400,
        headers: {
          "Content-Type": "application/json",
