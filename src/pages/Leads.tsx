...
export default function LeadsPage(){
  ...
  useEffect(()=>{ (async()=>{
    ...
    try{
      ...
      let role: Role = 'Junior'
      if (uid){
        const { data: me } = await supabase.from('advisors').select('role').eq('user_id', uid).maybeSingle()
        if (me?.role){ role = me.role as Role; setMeRole(role) }
      }

      // Carica usando il ruolo/uid rilevati (evita race sugli state set)
      await Promise.all([loadLeads(role, uid), loadAdvisors()])
    } catch(ex:any){ setErr(ex.message || 'Errore inizializzazione') }
    finally{ setLoading(false) }
  })() },[])

  /**
   * Carica i lead visibili in base al ruolo:
   * - Admin: tutti
   * - Team Lead: i propri + quelli dei Junior con team_lead_user_id = me
   * - Junior: solo i propri
   */
  async function loadLeads(roleOverride?: Role, uidOverride?: string){
    try{
      const role = roleOverride ?? meRole
      const uid  = uidOverride ?? meUid

      let ownerIds: string[] = []

      if (role === 'Admin'){
        const { data: all, error: e1 } = await supabase.from('advisors').select('user_id')
        if (e1) throw e1
        ownerIds = (all || []).map(a => a.user_id as string).filter(Boolean)
      } else if (role === 'Team Lead'){
        const { data: team, error: e2 } = await supabase
          .from('advisors')
          .select('user_id')
          .or(`user_id.eq.${uid},team_lead_user_id.eq.${uid}`)
        if (e2) throw e2
        ownerIds = (team || []).map(a => a.user_id as string).filter(Boolean)
      } else {
        ownerIds = uid ? [uid] : []
      }

      if (ownerIds.length === 0){ setLeads([]); return }
