...
export default function ResetPasswordPage(){
  ...
  // 1) Attiva la sessione di recovery/invite se presente nel fragment
  useEffect(()=>{ (async()=>{
    try{
      // Alcuni link usano #access_token&type=recovery, altri type=signup/invite
      const hash = typeof window !== 'undefined' ? window.location.hash : ''
      if (hash && hash.includes('access_token')) {
        // Crea la sessione dalla URL (Supabase v2)
        await supabase.auth.exchangeCodeForSession(hash)
      }
    }catch(e:any){
      console.error('exchangeCodeForSession error', e)
    }finally{
      // Verifica se ora c'è sessione (recovery attiva)
  ...
  async function onSave(e:React.FormEvent){
    ...
    try{
      ...
      setOk('Password aggiornata con successo. Reindirizzamento al login…')

      // Pulisci sessione “recovery” e vai al login
      await supabase.auth.signOut()
      // usa replace per evitare che l’utente torni indietro sulla pagina di reset con il back
      ...
      <div style={{ maxWidth:480, width:'min(92vw, 480px)', background:'#fff', border:'1px solid #eee', borderRadius:16, padding:24 }}>
        ...
        <form onSubmit={onSave} style={{ display:'grid', gap:10 }}>
          ...
          <button
            type="submit"
            disabled={saving}
            style={{ padding:'10px 12px', borderRadius:8, border:'1px solid #0b57d0', background:'#0b57d0', color:'#fff' }}
          >
