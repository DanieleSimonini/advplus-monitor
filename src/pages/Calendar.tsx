...
/**
 * Calendar.tsx — Agenda con vista mensile + vista settimanale
 * Patch: overflow-safe per i giorni con molti appuntamenti + toggle di vista
 * Mantiene la logica CRUD/Supabase già esistente.
 */
...
export default function CalendarPage() {
  ...
  // modali
  const emptyDraft: { id: string; lead_id: string; ts: string; mode: Mode; notes: string } = {
    id: '',
    lead_id: '',
    ts: '',
    mode: 'inperson',
  ...
  async function saveDraft() {
    if (!draft.lead_id) {
      alert('Seleziona un lead')
      return
    }
    if (!draft.ts) {
      alert('Imposta data/ora')
      return
    }
    ...
    const { data: rows } = await supabase
      ...
      .order('ts', { ascending: true })
    const leadMap = new Map((lds || []).map(x => [x.id, x] as const))
    setAppts(((rows || []) as any).map((r: any) => ({ ...r, lead: leadMap.get(r.lead_id) })) as Appointment[])
  }
  ...
  const apptsByDay = useMemo(() => {
    const filtered = selectedAdvisor ? appts.filter(a => a.lead?.owner_id === selectedAdvisor) : appts
  ...
  // utility local per ottenere chiave giorno
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'end', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button
              ...
              onClick={() =>
                setMonth(prev => {
                  ...
                  return monthKey(addMonths(new Date(y, m - 1, 1), -1))
                })
              }
            >
              {'‹'}
            ...
            <input
              ...
              onChange={e => {
                ...
                setActiveDate(new Date(y, m - 1, 1))
              }}
              style={ipt}
            />
            <button
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="brand-btn" onClick={() => setView('month')}>
              Mese
            </button>
            <button className="brand-btn" onClick={() => setView('week')}>
              Settimana
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <button className="brand-btn" onClick={() => openCreate(new Date())}>
            + Appuntamento
          </button>
        </div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    <button title="Nuovo appuntamento" onClick={() => openCreate(cell.date)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                      ＋
                    </button>
                  </div>
                    ...
                    {preview.map(a => (
                      <div key={a.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '6px 8px', background: '#f8fafc' }}>
                        <div style={{ fontSize: 12, fontWeight: 600 }}>
                          {new Date(a.ts).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })} · {MODE_OPTIONS.find(m => m.db === a.mode)?.label}
                        </div>
                        <div style={{ fontSize: 12, color: '#374151' }}>{labelLead(a.lead)}</div>
                        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                          <button title="Modifica" onClick={() => openEdit(a)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                            ✏️
                          </button>
                          <button title="Elimina" onClick={() => deleteAppt(a.id)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                            🗑️
                          </button>
                        </div>
                      </div>
                  ...
                  {hidden > 0 && (
                    <button
                      onClick={() => setOpenDayDate(cell.date)}
                      style={{ marginTop: 6, border: 'none', background: 'transparent', textAlign: 'left', cursor: 'pointer', fontSize: 12, color: '#0b57d0' }}
                    >
                      +{hidden} altri
                    </button>
                  )}
      ...
      {view === 'week' && (
        <div className="brand-card" style={{ ...box }}>
          {/* Header settimana: navigazione */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <button className="brand-btn" onClick={() => setActiveDate(d => addDays(d, -7))}>
                {'‹'}
              </button>
            ...
            <div>
              <button className="brand-btn" onClick={() => setActiveDate(new Date())}>
                Oggi
              </button>
            </div>
          </div>
                  ...
                  {days.map((d, i) => {
                    ...
                    return (
                      <div key={i} style={{ border: '1px solid var(--border,#eee)', borderRadius: 12, padding: 8, background: '#fff', minHeight: 220, display: 'flex', flexDirection: 'column' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, color: isToday ? '#0b57d0' : '#111' }}>
                            {d.getDate()}/{d.getMonth() + 1}
                          </div>
                          <button title="Nuovo appuntamento" onClick={() => openCreate(d)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                            ＋
                          </button>
                        </div>
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                          ...
                          {items.map(a => (
                            <div key={a.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', background: '#f8fafc' }}>
                              <div style={{ fontSize: 12, fontWeight: 600 }}>
                                {new Date(a.ts).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })} · {MODE_OPTIONS.find(m => m.db === a.mode)?.label}
                              </div>
                              <div style={{ fontSize: 12 }}>{labelLead(a.lead)}</div>
                              <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                                <button title="Modifica" onClick={() => openEdit(a)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                                  ✏️
                                </button>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, width: 420 }}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                <button className="brand-btn" onClick={closeEditor}>
                  Annulla
                </button>
                <button className="brand-btn" onClick={saveDraft}>
                  Salva
                </button>
              </div>
            </div>
      ...
      {/* Pannello "tutti gli appuntamenti del giorno" */}
      {openDayDate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'grid', placeItems: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, width: 520, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontWeight: 700, marginBottom: 12 }}>
              Appuntamenti ·{' '}
              {openDayDate.toLocaleDateString('it-IT', { weekday: 'long', day: '2-digit', month: '2-digit', year: 'numeric' })}
            </div>
            <div style={{ display: 'grid', gap: 8, overflow: 'auto' }}>
              {(apptsByDay.get(dayKey(openDayDate)) || []).map(a => (
                <div key={a.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', background: '#f8fafc' }}>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>
                    {new Date(a.ts).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })} · {MODE_OPTIONS.find(m => m.db === a.mode)?.label}
                  </div>
                  <div style={{ fontSize: 13 }}>{labelLead(a.lead)}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
              <button className="brand-btn" onClick={() => setOpenDayDate(null)}>
                Chiudi
              </button>
            </div>
          </div>
